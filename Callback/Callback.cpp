...
void callbackTemplate(Function func){func;}

// Callback made by me using Functor, may have flaws.
struct Functor
{
    void operator()(void* funct){funct;};
};
...
public:
    // Normal method
    void hit(){std::cout << "Been hit by fireball" << std::endl;}

    // operator () for class to be able to call it throgh Fireball();
    void operator()(){std::cout << "Hi, im the fireball class function" << std::endl;}
};
